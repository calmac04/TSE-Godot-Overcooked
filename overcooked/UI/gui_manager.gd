...
@onready var recipe_locs = get_parent()
...
func shift_recipes_left_if_needed():
	...
	while slots[0].get_child_count() == 0 and slots[1].get_child_count() == 1:
...
func remove(dish):
	...
	for slot in recipe_locs.get_children():
		if slot.get_child_count() == 1:
			print(slot.get_child(0).name)
			if slot.get_child(0).name == dish:
				slot.get_child(0)._finish()
				break
